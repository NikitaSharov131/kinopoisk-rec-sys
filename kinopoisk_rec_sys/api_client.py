...
from kinopoisk_rec_sys.models import (Genre, MovieQuery, PossibleValues,
                                      Rating, User)


def _assign_rating(rating: Rating):
    return {
        rating_kind: f"{rating_value}-10"
        for rating_value, rating_kind in zip(
            [rating.kp, rating.imdb], ["rating.kp", "rating.imdb"]
        )
        if rating_value
    }
...
class KinopoiskApiClient:
    ...
    def get_possible_genres(self) -> List[Genre]:
        ...
        if genres := self._get(
            endpoint, attrs.asdict(PossibleValues(field="genres.name")), "v1"
        ):
